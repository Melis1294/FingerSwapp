...
maskHairLocation = "images/JolieHair.jpg"
...
layout = [[sg.Text('Inserisci le immagini')],
          ...
          [sg.Text('HairMask', size=(8, 1)), sg.Input(), sg.FileBrowse()],
          [sg.Submit(), sg.Cancel()]]
...
myFaceAgedGreyScale = cv2.cvtColor(myFaceAged, cv2.COLOR_BGR2GRAY)
hairMask = cv2.imread(maskHairLocation)
hairMaskGrey = cv2.cvtColor(hairMask, cv2.COLOR_BGR2GRAY)
mask_blur_hair = cv2.GaussianBlur(hairMaskGrey, (51, 51), 0)

mask_hair = np.uint8(myFaceAgedGreyScale * (mask_blur_hair / 255))

greyHair = np.zeros_like(myFaceAged)
greyHair[:, :, 0] = np.uint8(myFaceAged[:, :, 0] * (1 - (mask_blur_hair / 255))) + mask_hair
greyHair[:, :, 1] = np.uint8(myFaceAged[:, :, 1] * (1 - (mask_blur_hair / 255))) + mask_hair
greyHair[:, :, 2] = np.uint8(myFaceAged[:, :, 2] * (1 - (mask_blur_hair / 255))) + mask_hair

cv2.imshow("Final", greyHair)
cv2.waitKey(0)
