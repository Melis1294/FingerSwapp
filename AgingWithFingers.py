...
def play_sound(filename,  #avvia la traccia audio passata come parametro
               wait):
    filepath = filename
    pygame.mixer.init()
    ...
    if wait:
        # se voglio far eseguire la traccia e far attendere l'esecuzione del codice seguente
        while pygame.mixer.music.get_busy() == True:
            continue

def show_hand(filename, frame, l_width, hand):     #mostra il numero fatto con la mano (in alto a sx)
    img_pre = cv2.imread(filename)                 #e la rotella con le varie scelte (in alto a dx)
    logo = cv2.resize(img_pre, (l_width, 100))

    if(hand == 1):
        start_roi_row = logo.shape[1]
        start_roi_col = logo.shape[0]
        image_roi = frame[0:start_roi_col, 0:start_roi_row]
    else:
    ...
    if(hand == 1):
        ...
        frame[0:start_roi_col, 0:start_roi_row] = result
        return frame
    else:
        return frame_bg, logo_bg, start_roi_col, start_roi_row

def main():
    global roi, punti_landmark_myFace, otherFace, punti_landmark_otherface, indexes_triangles, landmarks, convexhullMyFace, onlyMyFace, prev
    cap = cv2.VideoCapture(0)
    plt.ion()
    fig = plt.figure()
    fig.canvas.mpl_connect("close_event", lambda event: handle_close(event, cap))
    img = None
    fingers_up = 0  # Flag che serve per far partire il programma senza necessità che si abbiano le dita alzate.
    filenumber = 0  # Contatore per dare un titolo alle immagini salvate

    draw_frame = 1  # Flag per attivare/disattivare il HUD
    TIMER = int(4)
    font = cv2.FONT_HERSHEY_SCRIPT_COMPLEX
    first = 1   # Serve per stabilire l'intervallo tra ogni secondo nel conto alla rovescia del TIMER

    # Faccio i calcoli del logo solo una volta e non sempre, evitando così di appesantire il programma
    frame = grab_frame(cap)
    frame_bg, logo_bg, start_roi_col, start_roi_row = show_hand('images/rotation_wheel.png', frame, 100, 0)

    while cap.isOpened():
        myFace = grab_frame(cap)

        # Attiva l'HUD (logo, riquadro), non fa screen, è il caso in cui ho la mano alzata
        if draw_frame == 1:
            screen = 0
            result = cv2.add(frame_bg, logo_bg)
            myFace[0:start_roi_col, start_roi_row:frame.shape[1]] = result
        ...
        if screen == 0:
            ...
            try:
                # Prendo solo il contorno che ha l'area più grande
                ...
                for i in range(defects.shape[0]):
                    ...
                    # trovo la lunghezza di tutti i lati del triangolo
                    ...
                    # distanza tra punto e convex hull
                    d = (2 * ar) / a

                    # applico la regola del coseno qui
                    angle = math.acos((b ** 2 + c ** 2 - a ** 2) / (2 * b * c)) * 57

                    # ignora angoli > 90 e ignora punti molto vicini al convex hull (di solito dovuti al rumore)
                ...
                #casi in cui faccio con la mano un numero da 1 a 5
                if fingers == 1:
                    myFace = show_hand('images/h1.jpg',myFace, 70, 1)
                    ...
                    myFace = show_hand('images/h4.jpg',myFace, 70, 1)
                    ...
                    #Nascondi l'HUD
                    myFace = show_hand('images/h5.jpg',myFace, 70, 1)
                    draw_frame = 0
                else:
                    cv2.putText(myFace, 'Place fingers in rectangle', (0, 50), font, 1, (0, 0, 255), 3, cv2.LINE_AA)
                    fingers_up = 0
        ...
        if fingers_up == 1:
            ...
            if len(faces2) > 0:
                for face in faces2:
                    # Salvo in questa variabile la posizione x,y di tutti i punti della mia faccia.
                ...
                for triangle_index in indexes_triangles:
                    # Triangulazione dell'altra faccia
                    ...
                    # Triangulazione della mia faccia
                    ...
                    # Ricostruzione della faccia destinazione
            ...
            # Avvio la procedura per lo screen, con conto alla rovescia
            if screen == 1:
                if first == 1:
                    prev = time.time()
                    first = 0
                if TIMER > 0:
                    ...
                    # Posiziono numeri del timer nel centro dello schermo
                ...
                if TIMER < 0:
                    ...
                    play_sound('audios/shutter.mp3', True)
                    filenumber += 1
                    TIMER = int(4)
                    draw_frame = 1
                    first = 1
